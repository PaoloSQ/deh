...
function fixLinksInHtml(file) {
  ...
  // Buscar todas las rutas normalizadas y reemplazar por la real
  Object.keys(routeMap).forEach((norm) => {
    const real = routeMap[norm];
    // Reemplazo en href/src
    html = html.replace(
...
console.log(
  "Rutas normalizadas reemplazadas por rutas reales en todos los HTML.",
);
