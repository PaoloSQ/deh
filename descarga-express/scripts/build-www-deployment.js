...
function toDeployPageOutput(relPath) {
  ...
  if (posixRel === 'index.html') return path.join(DEPLOY_ROOT, `index${PAGE_TEMPLATE_EXTENSION}`);
  if (/\/index\.html$/i.test(posixRel)) {
    return path.join(DEPLOY_ROOT, posixRel.replace(/index\.html$/i, `index${PAGE_TEMPLATE_EXTENSION}`));
  }
...
function toSourceFileFromLocalUrl(urlPath) {
  ...
  if (cleanPath.startsWith('/img/')) return path.join(PUBLIC_ROOT, cleanPath.slice(1));
  if (cleanPath.startsWith('/css/')) return path.join(PUBLIC_ROOT, cleanPath.slice(1));
  if (cleanPath.startsWith('/js/')) return path.join(PUBLIC_ROOT, cleanPath.slice(1));
  if (cleanPath.startsWith('/assets/')) return path.join(PUBLIC_ROOT, cleanPath.slice(1));
  if (cleanPath.startsWith('/media/')) return path.join(PUBLIC_ROOT, cleanPath.slice(1));
  if (cleanPath.startsWith('/docs/')) return path.join(PUBLIC_ROOT, cleanPath.slice(1));
  if (cleanPath.startsWith('/_partials/wix-thunderbolt/dist/')) {
    return path.join(PUBLIC_ROOT, 'assets', 'js', 'services', 'wix-thunderbolt', 'dist', cleanPath.split('/').pop());
  }
  return null;
}
...
function rewriteStaticParastorageUrl(url) {
  try {
    ...
    if (ext === '.js' || ext === '.map' || !ext) return `${BETA_ORIGIN}/assets/js/${rel}`;
...
function rewriteAbsoluteUrl(value) {
  ...
  try {
    ...
    if (/^127\.0\.0\.1$/i.test(url.hostname) && /^\/www\.dehonline\.es(?:\/|$)/i.test(url.pathname)) {
...
function rewriteTextUrls(text) {
  ...
  next = next.replace(/https?:\/\/www\.dehonline\.es(\/[^"'`\s<)]*)?/gi, (_match, suffix = '') => `${BETA_ORIGIN}${suffix || ''}`);
  next = next.replace(/https?:\/\/127\.0\.0\.1:\d+\/www\.dehonline\.es(\/[^"'`\s<)]*)?/gi, (_match, suffix = '') => `${BETA_ORIGIN}${suffix || ''}`);
  next = next.replace(/https?:\/\/www\.dehonline\.es\/_partials\/wix-thunderbolt\/dist\/([^"'`\s<)]+)/gi, (_match, file) => `${BETA_ORIGIN}/_partials/wix-thunderbolt/dist/${file}`);
  next = next.replace(/https?:\/\/static\.parastorage\.com\/([^"'`\s<)]+)/gi, (match) => rewriteStaticParastorageUrl(match));
  next = next.replace(/https?:\/\/static\.wixstatic\.com\/media\/([^"'`\s<)]+)/gi, (_match, suffix) => `${BETA_ORIGIN}/assets/img/media/${suffix}`);
...
function walkObject(node) {
  ...
  if (!node || typeof node !== 'object') {
    if (typeof node === 'string') return rewriteAbsoluteUrl(rewriteTextUrls(node));
    return node;
  }
    ...
    if (key === 'htmlEmbeds' && Array.isArray(value)) {
      out[key] = value
        .filter((embed) => {
          ...
          return !/cdn\.jsdelivr\.net|auto\.srv791713\.hstgr\.cloud|fonts\.googleapis\.com|fonts\.gstatic\.com|visitor-analytics\.io|linkedin|sentry/i.test(html);
        })
...
function transformHtml(html) {
  ...
  $('link[href], script[src], iframe[src], img[src], source[src], source[srcset], form[action], a[href], meta[content]').each((_index, element) => {
  ...
  $('link[rel="preconnect"], link[rel="preload"], link[rel="dns-prefetch"]').each((_index, element) => {
    const href = $(element).attr('href') || '';
    if (shouldBlockExternalUrl(href) || /siteassets\.parastorage\.com/i.test(href)) {
      $(element).remove();
    }
  });

  $('script[type="application/json"]#wix-essential-viewer-model, script[type="application/json"]#wix-viewer-model').each((_index, element) => {
...
function collectLocalRefsFromText(text, kind) {
  ...
  if (kind === 'css') {
    ...
    while ((match = cssRe.exec(text))) {
      ...
      if (rewritten.startsWith(BETA_ORIGIN)) refs.add(new URL(rewritten).pathname);
    }
    return refs;
  }
  ...
  $('[href], [src], [srcset], [action], meta[content]').each((_index, element) => {
    for (const attrib of ['href', 'src', 'srcset', 'action', 'content']) {
      const value = $(element).attr(attrib);
      if (!value) continue;
      const parts = attrib === 'srcset' ? value.split(',').map((item) => item.trim().split(/\s+/)[0]) : [value];
...
function build() {
  ...
  while (assetQueue.size > 0) {
    ...
    if (ext === '.css' || ext === '.js') {
      ...
      for (const ref of collectLocalRefsFromText(transformed, ext === '.css' ? 'css' : 'js')) {
        assetQueue.add(ref);
      }
      continue;
    }
  ...
  fs.writeFileSync(path.join(DEPLOY_ROOT, 'PAGE-MANIFEST.json'), JSON.stringify(pageManifest, null, 2));
  fs.writeFileSync(path.join(DEPLOY_ROOT, 'DEPLOY-REPORT.json'), JSON.stringify(report, null, 2));
  fs.writeFileSync(
